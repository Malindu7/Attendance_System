...
# EXCEL EXPORT ROUTE
# URL: /export_excel
# Supports same filters as dashboard:
#   ?course=X  &  ?subject=Y  &  ?date=YYYY-MM-DD
# Downloads a formatted .xlsx attendance sheet
# ============================================================
@app.route('/export_excel')
def export_excel():
    # ── Read filters from URL (same as dashboard) ──────────
    sel_course  = request.args.get('course',  '')
    sel_subject = request.args.get('subject', '')
    sel_date    = request.args.get('date', datetime.now().strftime('%Y-%m-%d'))
    ...
    # ── Query MongoDB (same logic as dashboard) ─────────────
    query = {"timestamp": {"$gte": day_start, "$lte": day_end}}
    if sel_course:
        query["course"] = sel_course
    if sel_subject:
    ...
    # Deduplicate — keep first occurrence per student+subject
    seen, unique = set(), []
    for log in raw_logs:
        key = (log['name'], log['subject'])
        if key not in seen:
    ...
    thin  = Side(style="thin",   color="AABBD4")
    thick = Side(style="medium", color=BLUE_HEX)
    ...
    ws["A2"] = "General Sir John Kotelawala Defence University  |  CS31092 Digital Image Processing  |  Group 04"
    ws["A2"].font      = Font(name="Calibri", size=11, color="9EC5E8")
    ...
    ws["A4"] = filter_text
    ws["A4"].font      = Font(name="Calibri", size=11, bold=True, color=NAVY_HEX)
    ...
    headers = ["#", "Student Name", "Student ID", "Course", "Subject", "Time Marked"]
    col_widths = [5, 24, 20, 22, 28, 18]

    for col_idx, (header, width) in enumerate(zip(headers, col_widths), start=1):
        cell = ws.cell(row=5, column=col_idx, value=header)
        cell.font      = Font(name="Calibri", bold=True, size=11, color=WHITE_HEX)
        cell.fill      = blue_fill
    ...
    for row_num, record in enumerate(unique, start=1):
        excel_row = row_num + 5
        row_fill  = PatternFill("solid", fgColor=WHITE_HEX if row_num % 2 == 1 else GREY_HEX)

        # Parse name back: stored as "FIRSTNAME_LASTNAME" or "FIRSTNAME"
        raw_name   = record.get("name", "")
        # Try to split name and student ID — stored as "NAME" in DB
        # (full label from filename)
        parts      = raw_name.replace("_", " ").title()
        student_id = ""
        # If name contains student ID pattern like D/BCS/24/0010
        # it was stored separately during registration — use as-is
        name_clean = parts
    ...
    summary_row = len(unique) + 7
    ws.merge_cells(f"A{summary_row}:E{summary_row}")
    ws.cell(row=summary_row, column=1,
            value=f"Total Students Present: {len(unique)}")
    ws.cell(row=summary_row, column=1).font      = Font(name="Calibri", bold=True, size=11, color=NAVY_HEX)
    ws.cell(row=summary_row, column=1).fill      = light_fill
    ws.cell(row=summary_row, column=1).alignment = left
    ws.cell(row=summary_row, column=1).border    = cell_border

    ws.cell(row=summary_row, column=6,
            value=f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ws.cell(row=summary_row, column=6).font      = Font(name="Calibri", size=10, italic=True, color="6B7A99")
    ws.cell(row=summary_row, column=6).fill      = light_fill
    ws.cell(row=summary_row, column=6).alignment = center
    ws.cell(row=summary_row, column=6).border    = cell_border
    ...
    # ── Save to memory and send as download ────────────────
    output   = BytesIO()
    wb.save(output)
    output.seek(0)

    filename = f"Attendance_{sel_date}"
    if sel_course:  filename += f"_{sel_course.replace(' ', '_')}"
    if sel_subject: filename += f"_{sel_subject.replace(' ', '_')}"
    filename += ".xlsx"
